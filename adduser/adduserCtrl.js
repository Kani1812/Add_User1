...
//     }
    ...
    $scope.pushAllRoleValues = [];
    $scope.selectOptions = [];
    $scope.triggerMultipleRoles = function(selectVal) {
        $scope.selectedData = selectVal;
        return $http.post(BASEURL + RESTCALL.RoleSpecificRead, {
            "RoleID": $scope.selectedData
        }).then(function(response) {
           
            if(response.data){
               
               delete  response.data.EffectiveFromDate;
               delete  response.data.IsSuperAdmin;
               delete  response.data.Status;
                var flag = false; 
                if(response.data.RoleID){
                    flag = true
                    $scope.selectMultOptions.push(response.data)
                    $scope.pushAllRoleValues.push(response.data)
                    return;  
                }else{
                    flag = false; 
                }              
    ...
    setTimeout(function() {
        ...
        $('#RoleId').on('select2:select', function(e) {
           ...
           /* for (i in $scope.createData.UserRoleAssociation) {
            ...
            for (i in $scope.createData.UserRoleAssociation) {               
                $scope.selectedRoleId = $scope.createData.UserRoleAssociation[i].RoleID;
                $scope.triggerMultipleRoles($scope.selectedRoleId);
            }
         var values = $(this).val();        
        ...
        $('#RoleId').on('select2:unselect', function(e) {
            ...
            //     $scope.selectedRoleId = $scope.createData.UserRoleAssociation[k].RoleID;
            //     $scope.triggerMultipleRoles($scope.selectedRoleId, i);
            // }
            $timeout(function() {
                $scope.remoteDataConfig();
                //triggerSelectDrops();
